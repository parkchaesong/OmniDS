"""Building blocks vendored from NVlabs/FoundationStereo.

Copied verbatim (only import paths adapted) so that this repository runs
without an external FoundationStereo checkout on sys.path.

    Source : https://github.com/NVlabs/FoundationStereo
             core/submodule.py  -> _is_contiguous, LayerNorm2d, BasicConv,
                                   BasicConv_IN, ResnetBasicBlock, Conv2x_IN
             core/extractor.py  -> ResidualBlock
    Copyright (c) 2024-Present, NVIDIA Corporation & affiliates.

NOTE ON LICENSING: the upstream code is released under the NVIDIA Source Code
License, which permits non-commercial (research) use only. That is more
restrictive than this repository's MIT license -- review before redistributing.
"""
